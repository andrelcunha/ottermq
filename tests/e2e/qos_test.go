...
// TestQoS_ChangeLimit_MidConsume verifies that changing QoS mid-consumption works
func TestQoS_ChangeLimit_MidConsume(t *testing.T) {
	conn, err := amqp.Dial(brokerURL)
	if err != nil {
		t.Fatalf("Failed to connect: %v", err)
	}
	...
	queueName := testQueue + "-change-limit"
	...
	// Start with prefetch=2
	err = ch.Qos(2, 0, false)
	if err != nil {
		t.Fatalf("Failed to set initial QoS: %v", err)
	}
	...
	for i := 0; i < 2; i++ {
		...
		case msg := <-msgs:
			received = append(received, msg)
			t.Logf("Initial: %s", string(msg.Body))
		case <-time.After(1 * time.Second):
			t.Fatalf("Timeout")
		}
	}
	...
	select {
	case <-msgs:
		t.Error("Received message beyond prefetch=2 limit")
	case <-time.After(500 * time.Millisecond):
		t.Log("Correctly limited at prefetch=2")
	}

	// Change QoS to prefetch=5
	err = ch.Qos(5, 0, false)
	if err != nil {
		t.Fatalf("Failed to change QoS: %v", err)
	}

	t.Log("Changed QoS to prefetch=5")

	// Should now receive 3 more messages (to reach new limit of 5)
	for i := 0; i < 3; i++ {
		select {
		case msg := <-msgs:
			t.Logf("After QoS change: %s", string(msg.Body))
		case <-time.After(1 * time.Second):
			t.Errorf("Expected message %d after increasing prefetch limit", i+1)
		}
	}
}
